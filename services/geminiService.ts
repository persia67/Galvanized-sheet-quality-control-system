...
// Using the specialized vision model for industrial inspection
  ...
  try {
    const prompt = `
      ...
      پاسخ را دقیقاً در فرمت JSON زیر برگردانید:
      {
        ...
        "defects": [
           { "type": "نام عیب به فارسی", "severity": "Low" | "Medium" | "High", "description": "توضیح کوتاه" }
        ],
    ...
    if (result.grade.includes('2')) finalGrade = SteelGrade.Grade2;
    if (result.grade.includes('3')) finalGrade = SteelGrade.Grade3;
